# Hi there 👋

👩‍💻 For docs of our products, check out our docs [site](https://docs.hexfellow.com) and [home page](https://hexfellow.com).
🧙 If you've got any questions, feel free to open a new discussion at github😁. Or, join our [Discord](https://discord.gg/HdGceFwRCj) if you want.

For most of our products, you have three layers of API that can be used:
1. Officially Supported [Websocket and UDP API protocol](https://github.com/hexfellow/proto-public-api), with [examples](https://github.com/hexfellow/robot-demos)
2. Community maintained Python Driver(Based on [Websocket and UDP API protocol](https://github.com/hexfellow/proto-public-api)): [Python Driver](https://github.com/hexfellow/hex_device_python)
3. Community maintained [Python Demos](https://github.com/hexfellow/hex_zmq_servers) based on [Python Driver](https://github.com/hexfellow/hex_device_python)

## Other Community maintained projects:
> These projects are community maintained. If you encountered any problems, please reach for devs directly or open discussion/issue.

ROS package for Base: [https://github.com/hexfellow/hex_vehicle](https://github.com/hexfellow/hex_vehicle)
